...
7. ☐ ⚠️ **Data safety form** (App content → Data safety): declare data collected/shared, that it's stored on-device, **not shared**, encrypted in transit if any network calls, and that the user **can request deletion** (your in-app reset). Must match the privacy policy. Health data = sensitive. Crash/diagnostics: the CatDoes Watch SDK is present but **disabled for v1** (`catdoes.watch.ts` early-returns), so no diagnostics leave the device — do **not** declare 'Diagnostics' in the Data Safety form. If re-enabled later, declare 'Diagnostics' in Data Safety and name CatDoes in the privacy policy.
